...
public enum TutorialStateV1 {
    ...
    QUEST_GUIDE_DIALOGUE(new NpcWorker(a -> a.getName().equals("Quest Guide"), new MovementWorker(new Position(3086, 3123, 0))), 200, 220, 240),
    ...
    TutorialStateV1(Worker worker, int... varps) {
        this.varps = varps;
        this.worker = worker;
    }

    public static TutorialStateV1 getValidState() {
        return Arrays.stream(values()).filter(TutorialStateV1::isInVarp).findFirst().orElse(null);
    }

    public static boolean isInVarp(TutorialStateV1 state) {
        return Arrays.stream(state.getVarps()).anyMatch(a -> a == Varps.get(QuestType.TUTORIAL_ISLAND_V1.getVarp()));
    }
